...
Monthly RSI (Wilder's method), and flags stocks where:
    ...
    Weekly RSI  < 30   (oversold on the weekly timeframe)
...
<style>
  ...
  .download {{ display:inline-block; margin-bottom:20px; padding:10px 16px; background:#2563eb; color:#fff; text-decoration:none; border-radius:6px; font-weight:600; }}
  table {{ border-collapse: collapse; width:100%; background:#171a21; }}
  th, td {{ padding:8px 12px; text-align:left; border-bottom:1px solid #2a2e37; font-size:0.9rem; }}
  th {{ background:#1f4e78; color:#fff; position:sticky; top:0; }}
  tr:hover {{ background:#20242c; }}
  .empty {{ color:#9aa0a6; padding:16px 0; }}
</style>
</head>
<body>
...
def send_telegram(matches: list[dict], report_url: str) -> None:
    ...
    if matches:
        lines = [f"*Nifty 500 RSI Screener* \u2014 {len(matches)} match(es)\n"]
        lines.append("Weekly RSI < 30, Monthly RSI > 40\n")
        for r in matches[:15]:
            lines.append(
                f"`{r['symbol']:<12}` W:{r['weekly_rsi']} M:{r['monthly_rsi']} "
                f"CMP:{r['current_price']} 52WH:{r['down_from_52w_high_pct']}%"
            )
        if len(matches) > 15:
            lines.append(f"...and {len(matches) - 15} more in the report.")
        if report_url:
            lines.append(f"\n[Full report]({report_url})")
        text = "\n".join(lines)
    else:
        text = "*Nifty 500 RSI Screener* \u2014 no stocks matched this run."
